...
class PluginPosTable extends Doctrine_Table {
    ...
    public function getResultSearchRentHouseQuery($position) {
        ...
        if ($position['min_price'] || $position['max_price']) {
            if($position['min_price'] && $position['max_price']){
                $q->andWhere('r.const_price_ranges > ? AND r.const_price_ranges <= ?',array($position['min_price'], $position['max_price']) );
            }
            elseif($position['min_price']){
                $q->andWhere('r.const_price_ranges > ?',$position['min_price']);
            }
            elseif($position['max_price']){
                $q->andWhere('r.const_price_ranges <= ?',$position['max_price']);
            }
        }
        if ($position['area']) {
            $area = $position['area'];
    ...
    public function getRentHouse($page = 1, $size = 10, $position = null) {
        $q = $this->getResultRentHouseQuery($position);
        return $this->getPager($q, $page, $size);
    }

    public function getResultRentHouseQuery($position) {
        $q = $this->createQuery('p');
        $q->innerJoin('p.PosRentHouse r');
        $q->where('p.is_public = ?', self::is_public);
        $q->andWhere('lat < ?', $position['max_lat']);
