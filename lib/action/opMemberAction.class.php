...
abstract class opMemberAction extends sfActions
{
  ...
  public function executeLogin($request)
  {
    ...
    if ($request->hasParameter('authMode'))
    {
      ...
      $this->getUser()->setFlash('notice', sfContext::getInstance()->getI18N()->__('Login Error'));
    ...
    {
      $this->getUser()->setFlash('notice', sfContext::getInstance()->getI18N()->__('Please login to visit this page'));
    }
    return sfView::SUCCESS;
  }
  ...
  public function executeSearch($request)
  {
    ...
    /* count total member*/
    $count = Doctrine::getTable('Member')->createQuery();
    $this->total_member = $count->count();
  ...
  public function executeConfig($request)
  {
    ...
    sfConfig::set('sf_nav_type', 'config_member');
    sfConfig::set('opLeftSideBar', 'configMember');
    $this->categoryName = $request->getParameter('category', null);
    if ($this->categoryName)
    {
      $this->forward404Unless(array_key_exists($this->categoryName, $this->categories), sfContext::getInstance()->getI18N()->__('Undefined category'));
  ...
  public function executeDelete($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        $this->getUser()->setFlash('notice', sfContext::getInstance()->getI18N()->__('Logout Success'));
        $this->getUser()->logout();
